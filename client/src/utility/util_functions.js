			...
			? state.cartItems.map(
					({ productId, quantity }) =>
						productId === product_id
							? quantity + incOrDec ? { productId, quantity: quantity + incOrDec } : null
							: { productId, quantity }
				)
...
export const removeItemFromCart = (state, product_id) => {
	return {
		...state,
		cartItems: state.cartItems.filter((cartItem) => cartItem.productId !== product_id)
	};
};
...
export const filterByStockAndDelivery = (cartState, productArray) => {
	...
	const inStockProducts = cartState.showAllProducts ? productArray : productArray.filter((item) => item.stock_quantity>0);

	return !cartState.fastDelivery ? inStockProducts : inStockProducts.filter((item) => item.isFastDeliveryAvailable);
};

export const showPriceRange = (state, productArray) => {
	return productArray.filter((product) => Number(product.price) <= state.priceRangeMaxValue);
};
