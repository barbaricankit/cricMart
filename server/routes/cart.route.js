...
const { findCart, checkNewCart, checkProductIsInCart, updateCart } = require('../utils/cart_util_func');

router
	.route('/:userId/cart')
	.get(findCart, checkNewCart, async (req, res) => {
		const { cart } = req;
		const { products } = await cart.populate({ path: 'products.productId' }).execPopulate();
		res.status(200).json({
			success: true,
			products
		});
	})
	.post(findCart, checkNewCart, checkProductIsInCart, updateCart, async (req, res) => {
